...
python3 testing/test_data_synthesizer.py 2>logs/synthetic.log | \
daq_core/rebuffer.out 0 2> logs/rebuffer.log &

# Decimator - Thread 1
chrt -f 99 daq_core/decimate.out 2> logs/decimator.log &

# Delay synchronizer - Thread 2
python3 daq_core/delay_sync.py 2> logs/delay_sync.log &

# Hardware Controller data path - Thread 3
sudo python3 daq_core/hw_controller.py 2> logs/hwc.log &
...
if [ $out_data_iface_type = eth ]; then
    ...
    daq_core/iq_server.out 2>logs/iq_server.log &
...
#sleep 3
#python3 daq_core/iq_eth_sink.py 2>logs/iq_eth_sink.log &
